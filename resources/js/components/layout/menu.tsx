...
import { MenuGroup, SharedData } from '@/types';
        ...
        <ScrollArea className="[&>div>div[style]]:!block">
            <nav className="mt-8 h-full w-full">
                <ul className="flex min-h-[calc(100vh-48px-36px-16px-32px)] flex-col items-start space-y-1 px-2 lg:min-h-[calc(100vh-32px-40px-32px)]">
                    {menuList.map(({ groupLabel, menus }:MenuGroup, index) => (
                            ...
                            <Tooltip delayDuration={100}>
                                <TooltipTrigger asChild>
                                    <Button onClick={() => {}} variant="outline" className="mt-5 h-10 w-full justify-center">
                                        ...
                                        <p className={cn('whitespace-nowrap', isOpen === false ? 'hidden opacity-0' : 'opacity-100')}>Sign out</p>
                                    </Button>
                                </TooltipTrigger>
